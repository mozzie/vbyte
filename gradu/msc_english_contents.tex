...
$Rank$ method is superior when dataset contains mostly small integers. With larger integers, multiple $rank$ calls are needed and $select$ method performs better.
...
For example, if block length is set to 4 and 42 is a 16 bit unsigned integer that is to be encoded. The standard 8-bit representation of 42 is 
...
which is the number 42 compressed with VB encoding and block length of 4. A pseudo code with block length 4 is shown in Figure~\ref{vbyte_enc}.
...
\section{VB decoding}
VB decoding is technically just reversing the encoding steps: chunks are read until a chunk with 1 as continuation bit is found. Continuation bits 
are removed from all the chunks and the blocks are concatenated to form the original number. An example implementation of VB decoding with block length of 7 is shown in Figure~\ref{vbyte_dec}.

As in the encoding example, block length is 4, the encoded message 
is \texttt{00010 11000} and n is a 16 bit unsigned integer. The block from the first chunk is extracted and added to $n$, making $n = $\texttt{10} (bit representation of 2).
...
$n = $\texttt{101000} (bit representation of 42). Because the continuation bit of this chunk is 1, decoding for this number is complete. 
