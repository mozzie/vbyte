...
In this thesis, an alternative solution that instead makes use of a $select$ data structure is proposed and explained in detail. The proposed method does not use data 
...
element ends and a new one begins. $Rank$ and $select$, the two bit array operations were also explained. Existing solution for direct access with VB using $rank$ was explained. A new solution for direct 
access with $select$ was proposed and results were compared between the two solutions. A method of extracting an array of data with VB encoding was proposed for both $rank$ and $select$.
...
consists of just a few bit operations. The amount of $rank$ queries depend on the length of the subarray and the average size of the elements. Therefore even a small amount of large elements in an array 
otherwise filled with small elements causes the runtime to go up. This leads to the conclusion that the $select$ method, if not strictly superior, is a strong contender and a good option for decompressing 
a subarray.
