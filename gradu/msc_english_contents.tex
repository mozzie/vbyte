...
The $rank$ method is found superior when dataset contains mostly small integers. With larger integers, the decoding needs multiple $rank$ calls and the new $select$-based 
method performs better. Additionally, the proposed $select$ method offers fast access to the next or previous element.
...
A single direct access with $select$ needs to calculate the length of the desired element. This is done by reading a 64-bit word from the continuation bit array, bit-shifting until the bit representing 
...
The computer runs Ubuntu 14.04.4 (3.13.0-91-generic x86\_64). The codes were compiled with \texttt{g++-7 -std=c++11 -DNDEBUG -O3} and with libraries \texttt{-lsdsl -ldivsufsort -ldivsufsort64}.
...
library \citep{gbmp2014sea}. Each implementation is compressing and decoding uint64\_t type integers. The $rank$ and $select$ implementations are explained in \autoref{chap:implementations}.
...
number of large integers within 1000 integers and the y-axis has the average time of fetching 50 subsequent numbers from an initially random location. 
...
TODO:
 ...
 - add subarray examples / visualization

 - write more about subarray access in results
