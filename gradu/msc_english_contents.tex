...
The most intuitive way to calculate block length of i-th element is to subtract $select_1$(i) from $select_1$(i+1). This however causes an additional 
...
element is on the rightmost end and then counting the trailing zeros. This counts the number of blocks in the i-th element that do not have the continuation bit set. The number is incremented by one to 
count in the ending block with the continuation bit.
...
After locating and decompressing one value from a compressed data structure, decompressing the subsequent elements is significantly easier. Calculating the memory location of the next element is not 
needed because the next compressed value is usually located next to the previously fetched element. This is especially beneficial to the VByte decoding with $select$ method, where calculating the start 
...
Single direct access with $select$ needs to calculate the length of the wanted element. This is done by reading a 64-bit word from the continuation bit array, bit-shifting until the bit representing 
...
and then counting trailing zeros. Once the word is shifted through, another word is read and process is continued.
...
When a new block level is reached first time during a subarray query, the index is fetched with $rank_1$ as normal, and the result of the query is stored. Next time the same level is reached, the
previous stored is incremented by one and that value is used instead of the $rank_1$ query. The amount of $rank_1$ calls needed is equal to the number of $rank_1$ calls needed to decompress the largest 
...
Six different VB decoding algorithms were compared. The Bris-algorithms are from \citep{Bri09}. All algorithms are implemented using data structures and functions from the SDSL 
library \citep{gbmp2014sea}. Each implementation is compressing and decoding uint64\_t type integers.

\begin{itemize}
  \item Bris4 - Bris implementation with block size 4, using rank support v
...
The 8-bit versions tend to perform better than the 4-bit versions. This is because 8-bit version stores more data in one block and less block readings are required. However, 4bit versions achieve 
...
The v5 version of rank naturally has worse performance because more calculations are needed in the last phase of calculating the rank sum. As a trade-off, it requires 
...
Table~\ref{table:results8bit} and Table~\ref{table:results4bit} show memory size requirements for $rank$ and $select$ data structures with data sets consisting
 of 50 million 64-bit numbers. The compressed size column has a sum of continuation bit array and the data blocks.
...
size allows a better data compression in some data sets and therefore a smaller total size. This can be seen when comparing the two forementioned figures: All 4-bit versions of the algorithms require
...
Figure~\ref{graph:subarray} shows results for subarray fetching. The data sets used contained mostly 4-bit integers and a few 32-bit integers randomly among them. The number in the x-axis corresponds to the 
number of large integers within 1000 integers and the y-axis has the average time of fetching 50 subsequent numbers from random location. 
...
\begin{axis}[
    ...
    ytick={300,400,500,600,700,800,900,100},
    legend pos=north west,
    ymajorgrids=true,
    grid style=dashed,
    width=13cm, height=8cm
...
\end{axis}
\end{tikzpicture}
\caption{Rank and select performances with subarray fetching} \label{graph:subarray}
\end{figure}
...
less manipulation because the data is already stored in the correct order, but calculating the integer block lengths might benefit from SIMD.
