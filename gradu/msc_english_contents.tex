...
data compression algorithms are inefficient, because data must be decompressed from the beginning to the point where the sought element is located. 
...
In this thesis, a novel alternative solution that instead makes use of a $select$ data structure is proposed and explained in detail. The underlying question is if a $select$ based method is viable for random 
...
Smaller block lengths can yield a better compression rate at the cost of more bit manipulation and therefore possibly slower decompression, while bigger block lengths need less bit manipulation and 
...
$B$ = \texttt{0100 1101 \underline{1}011}, $select_1$(5) returns 8 (underlined) and therefore $rank_1$(8) returns 4. 
...
as the needed indexing array. In this case, $rank_1$(i) from $C$ would give the number of end blocks before the i-th index and $select_1$(i) would give the location of the ending block 
...
64-bit word. For every 512 bits, this implementation requires 64 bits for the superblock and another 64 bits for the relative blocks, resulting in additional 128/512 = 25\% space.
...
The $select$ data structure works in a similar way to rank. The index location of every 4096th set bit is stored in the superblock and the location of every 64th set bit is stored 
...
After the data has been arranged this way, a $rank$ data structure is built for each bit array. The data structure does not require additional space on top of that used by the normal VB 
encoding apart from that used by the $rank$ structure. Figure~\ref{bris_ds} contains a visualization of the data structure. The C array is the original VB encoded data, which is split into separate block arrays $A_k$ and 
...
\end{tikzpicture}
\caption{Data structure by \citeauthor{Bri09}, visualized}\label{bris_ds}
...
Using $select$ on the continuation bit array to achieve direct access is more intuitive than using $rank$. The element locations are already marked with 1's 
...
To use $select$ with VB, continuation bits need to be separated from chunks to their own bit array and a $select_1$ structure built over it. Because every compressed element has 1
...
For example, let the integer bit length be 32 and the original data be compressed to 5 blocks of 4 bits. First, the byte location of the first block is needed. The index location $x$ of the desired block
 is calculated $x = select_1$($i$-1) + 1, but since the block length differs from byte length, the starting byte location $s$ needs to be calculated $s = (x \times 4) \div 8$, the latter operand being for integer division.
The first block might not be at the start of the byte $s$. This is why offset $o = (x \times 4)$ $mod$ $8$ is needed (in the example case, $o$ is equal to either 0 or 4). Then a 32-bit word $w$ is read from memory from byte location $s$. 
...
The most intuitive way to calculate the block length of the $i$-th element is to subtract $select_1$($i$-1) from $select_1$($i$). This however causes an additional 
...
After locating and decompressing one value from a compressed data structure, decompressing the subsequent elements can become significantly easier. Calculating the memory location of the next element may not 
...
\draw [decorate,decoration={brace,amplitude=5pt,mirror,raise=10ex}]
  (1.8,0) -- (9.5,0) node[midway,yshift=-6em]{word $w$};
\end{tikzpicture}
...
The length of the desired element $l$ and the location $i$ can now be used to decompress the desired element. For the next element, $i$ is incremented by $l$ and $w$ is bit shifted $l$ bits and the process is 
...
For the location of the first block of the second element, $P[1]$ is checked. It is set, so the value it contains is increased by one and used as the index. The bit array shows that this element is only 
one block long. The first two blocks of the third element are fetched the same way. $P$[2] is unset and a  $P$[2] = $rank_0$($B_2$,0) is used to fetch the index. The fourth element does not reach to a new 
block level, so each block location is fetched from $P$.
...
[node distance=0pt,
 ...
 Y/.style = {rectangle, draw,% styles of nodes in string (chain)
                minimum width=4ex, minimum height=3ex,
                outer sep=0pt, on chain}
 ]
...
\node [Y, right=55ex of a1] (p1){-1};
\node [left=2ex of p1]{\textbf{Initial:}};
\node[Y] (p1_2) {-1};
\node[Y] (p1_3) {-1};
\node[Y] (p1_4) {-1};
\node [above=1ex of p1]{\textbf{$P_1$}};
\node [above=1ex of p1_2]{\textbf{$P_2$}};
\node [above=1ex of p1_3]{\textbf{$P_3$}};
\node [above=1ex of p1_4]{\textbf{$P_4$}};

\node [Y, below=3ex of p1] (p2){0};
\node [left=2ex of p2]{\textbf{After $C_1$:}};
\node[Y] (p2_2){0};
\node[Y] (p2_3){-1};
\node[Y] (p2_4){-1};

\node [Y, below=3ex of p2] (p3){1};
\node [left=2ex of p3]{\textbf{After $C_2$:}};
\node[Y] {0};
\node[Y] {-1};
\node[Y] {-1};

\node [Y, below=3ex of p3] (p4){2};
\node [left=2ex of p4]{\textbf{After $C_3$:}};
\node[Y] {1};
\node[Y] {0};
\node[Y] {-1};

\node [Y, below=3ex of p4] (p5){3};
\node [left=2ex of p5]{\textbf{After $C_4$:}};
\node[Y] {2};
\node[Y] {0};
\node[Y] {-1};
...
Each implementation compresses and decompresses 64-bit unsigned integers. The $rank$ and $select$ implementations are explained in detail in \Cref{chap:implementations} 
and \Cref{chap:select_impl}, respectively. 
...
of one million $rank$ and $select$ calls with different average integer lengths. The effect of multiple $rank$ calls is visible, as is the single $select$ call behavior, which leads to a much flatter curve.

The 8-bit versions tend to perform better than the 4-bit versions. This is because the 8-bit version stores more data in one block and so less block readings are required. However, the 4-bit versions achieve 
...
\end{axis}
\end{tikzpicture}
\caption{$Rank$ and $select$ performance with subarray fetching. The scale of the y-axis was selected to clarify the difference of the results.} \label{graph:subarray}
\end{figure}

\chapter{Conclusions and Future work}

This thesis explained variable-byte encoding, a compression method that stores the data in fixed length blocks and omits empty leading blocks. A continuation bit is added to each block to signal where an 
element ends and a new one begins. $Rank$ and $select$, two array operations were explained and efficient implementations over bit arrays were introduced in detail. 

An efficient solution to direct access to any element in a compressed data structure was previously introduced by \citeauthor{Bri09}. Their solution uses VB encoding and a $rank$ data structure. 
...
a machine word with just 4 instructions \citep{Pan17a}. They have been tested with the $select$ algorithm of the SDSL-library (which is also used in the proposed method of this thesis) 
and show an improvement of 2-15\% \citep{Pan17b}. Additionally, a trailing zero count function is used to calculate the length of the element. 
Some improvement in speed may happen if the implementation is changed to use the \textsc{TZCNT} machine instruction instead.


One more interesting improvement to VB encoding is to see whether the compressed numbers can be changed. This is trivial until the block length of the compressed number changes. Then an additional data structure or 
