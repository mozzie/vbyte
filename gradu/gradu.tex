...
data structures, which is not possible with traditional compression methods.
...
denote whether the compressed integer continues to the next byte or not. An existing solution using \textsc{rank} data structure performs well in this given task. This thesis introduces an 
alternative solution using \textsc{select} data structure and compares the two implementations. An experimentation is also done on retrieving a subarray from the compressed data structure.
